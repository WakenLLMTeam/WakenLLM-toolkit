...
class DeerFlowClient:
    ...
    def _get_runnable_config(self, thread_id: str, **overrides) -> RunnableConfig:
        ...
        configurable = {
            ...
            "rag_resources": overrides.get("rag_resources", []),
            "metadata_condition": overrides.get("metadata_condition"),
        }
    ...
    def _ensure_agent(self, config: RunnableConfig):
        ...
        rag_resources = cfg.get("rag_resources", [])
        metadata_condition = cfg.get("metadata_condition")
        key = (
            cfg.get("model_name"),
            cfg.get("thinking_enabled"),
            cfg.get("is_plan_mode"),
            cfg.get("subagent_enabled"),
            json.dumps(rag_resources, sort_keys=True, ensure_ascii=False, default=str),
            json.dumps(metadata_condition, sort_keys=True, ensure_ascii=False, default=str),
        )
    ...
    def _get_tools(
        *,
        ...
        rag_resources: list[dict[str, Any] | Resource] | None = None,
        ...
        for item in rag_resources or []:
            if isinstance(item, Resource):
                normalized_resources.append(item)
                continue
            if isinstance(item, dict):
                try:
                    normalized_resources.append(Resource.model_validate(item))
                except Exception:
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs: Override client defaults (model_name, thinking_enabled,
                plan_mode, subagent_enabled, recursion_limit, rag_resources,
                metadata_condition).
