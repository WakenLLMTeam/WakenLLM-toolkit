...
class MemoryConfig(BaseModel):
    ...
    storage_path: str = Field(
        ...
        description=(
            ...
            "Use `memory.json` for the default Deer Flow 2 location at "
            "`backend/.deer-flow-2/memory.json`; setting `.deer-flow-2/memory.json` "
            "would create a nested `{base_dir}/.deer-flow-2/memory.json` path."
        ),
    )
