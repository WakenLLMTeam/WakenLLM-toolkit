...
- **Location**: `config.yaml` should be in `deer-flow-2/` (project root), not `deer-flow-2/backend/`
...
3. `deer-flow-2/config.yaml` (parent directory - **recommended location**)

**Recommended**: Place `config.yaml` in project root (`deer-flow-2/config.yaml`).
