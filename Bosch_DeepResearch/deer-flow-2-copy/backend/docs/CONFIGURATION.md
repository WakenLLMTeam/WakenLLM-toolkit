...
sandbox:
  ...
  port: 8080
  auto_start: true
  container_prefix: deer-flow-2-sandbox
...
**How Skills Work**:
- Skills are stored in `deer-flow-2/skills/{public,custom}/`
...
The configuration file should be placed in the **project root directory** (`deer-flow-2/config.yaml`), not in the backend directory.
...
4. `config.yaml` in parent directory (project root: `deer-flow-2/`)
...
- Ensure `config.yaml` exists in the **project root** directory (`deer-flow-2/config.yaml`)
...
### "Skills not loading"
- Check that `deer-flow-2/skills/` directory exists
