...
class CitationMiddleware(AgentMiddleware[CitationMiddlewareState]):
    ...
    def _merge_title_snippet_raw_for_publication_infer(url: str) -> bool:
        ...
        return (
            ...
            or CitationMiddleware._is_youtube_url(url)
    ...
    def _year_from_published_or_snippet(
        ...
        *,
        ...
        if sn:
            ...
            # then take the **maximum** of that band — usually the most recent article date.
            years = sorted(
                {
                    int(x)
    ...
    def _extract_tool_citations_with_snippets(messages: list) -> list[dict]:
        ...
        for msg in messages:
            ...
            logger.debug("CitationMiddleware: processing ToolMessage tool_name=%s content_preview=%s", tool_name, content[:100] if content else "")
            ...
            if not citation_matches:
                ...
                json_citations = CitationMiddleware._citations_from_plain_json_search_results(
                    content, tool_name
                )
                logger.debug("CitationMiddleware: extracted %d JSON citations from %s", len(json_citations), tool_name)
    ...
    @staticmethod
    def _enrich_reddit_citation_dates(cit: dict, *, source_url: str, snippet: str) -> None:
        """Fill missing calendar date and/or year from Reddit snippet/raw_content."""
        if "reddit.com" not in (source_url or "").lower():
            return
        pub = (cit.get("published_date") or "").strip()
        yr = (cit.get("year") or "").strip()
        if pub and yr:
        ...
        # Try to extract date from raw_content first
        raw_blob = ((cit.get("raw_content") or "") + "\n" + (snippet or "")).strip()
        if raw_blob:
            try:
                from deerflow.utils.publication_date import infer_reddit_post_calendar_date
                got = infer_reddit_post_calendar_date(raw_blob)
                if got:
                    cit["published_date"] = got
                    cit["year"] = CitationMiddleware._year_from_published_or_snippet(
                        got,
        ...
        if snippet:
            try:
                ...
                if years:
                    # Take the maximum year (most recent)
                    year_int = max(int(y) for y in years if 1900 <= int(y) <= 2100)
                    cit["year"] = str(year_int)
            except Exception:
                pass

    @staticmethod
    def _enrich_instagram_citation_dates(cit: dict, *, source_url: str, snippet: str) -> None:
        """Fill missing calendar date and/or year from Instagram JSON in raw_content / snippet."""
        if "instagram.com" not in (source_url or "").lower():
            return
        pub = (cit.get("published_date") or "").strip()
        yr = (cit.get("year") or "").strip()
        if pub and yr:
    ...
    @staticmethod
    def _enrich_generic_citation_dates(cit: dict, *, source_url: str, snippet: str) -> None:
        """Fallback: extract year from snippet if no date found yet."""
        yr = (cit.get("year") or "").strip()
        if yr:
            return
    ...
    @staticmethod
    def _enrich_citation_dates_multi_strategy(cit: dict, *, source_url: str, snippet: str) -> None:
        """Multi-strategy date extraction with fallbacks for all URLs."""
        yr = (cit.get("year") or "").strip()
        if yr:
            return

        # For Reddit, prioritize snippet year (post IDs decode incorrectly)
        if "reddit.com" in (source_url or "").lower():
            if snippet:
                try:
                    years = re.findall(r"\b((?:19|20)\d{2})\b", snippet)
                    if years:
        ...
        # For X/Twitter, extract year from snippet or raw_content (not status ID)
        if "x.com" in (source_url or "").lower() or "twitter.com" in (source_url or "").lower():
            # Try snippet first
            if snippet:
                try:
                    years = re.findall(r"\b((?:19|20)\d{2})\b", snippet)
                    ...
                    pass
            # Fall through to Strategy 1 (raw_content) for X tweets

        # For tesery.com, try snippet/URL first, then raw_content
        if "tesery.com" in (source_url or "").lower():
            # Try snippet first
            if snippet:
                try:
                    years = re.findall(r"\b((?:19|20)\d{2})\b", snippet)
            ...
            # Try URL
            try:
                years = re.findall(r"\b((?:19|20)\d{2})\b", source_url)
                if not years:
                    years = re.findall(r"(20\d{2})[\.\-_/]", source_url)
                if years:
                    from datetime import datetime
                ...
                pass
            # Fall through to Strategy 1 (raw_content) if snippet/URL fail

        # Strategy 1: Try to extract from raw_content using infer_publication_calendar_date
        raw_blob = (cit.get("raw_content") or "").strip()
        if raw_blob:
            try:
                from deerflow.utils.publication_date import infer_publication_calendar_date
                got = infer_publication_calendar_date(raw_blob[:1_000_000], source_url=source_url)
        ...
        # Strategy 2: Handle relative time (e.g., "Posted 2 months ago")
        try:
            from datetime import datetime, timedelta
            relative_patterns = [
                (r'posted\s+(\d+)\s+days?\s+ago', lambda m: datetime.now() - timedelta(days=int(m.group(1)))),
                (r'posted\s+(\d+)\s+weeks?\s+ago', lambda m: datetime.now() - timedelta(weeks=int(m.group(1)))),
                (r'posted\s+(\d+)\s+months?\s+ago', lambda m: datetime.now() - timedelta(days=int(m.group(1))*30)),
                (r'posted\s+(\d+)\s+years?\s+ago', lambda m: datetime.now() - timedelta(days=int(m.group(1))*365)),
            ]

            combined_text = (raw_blob + " " + snippet).lower()
            for pattern, calc_date in relative_patterns:
                match = re.search(pattern, combined_text)
                if match:
                    dt = calc_date(match)
                    cit["year"] = str(dt.year)
                    return
        except Exception:
            pass

        # Strategy 3: Extract year from snippet
        if snippet:
            try:
                years = re.findall(r"\b((?:19|20)\d{2})\b", snippet)
                if years:
        ...
        # Strategy 4: Extract year from URL path (last resort)
        try:
            years = re.findall(r"\b((?:19|20)\d{2})\b", source_url)
            if not years:
                years = re.findall(r"(20\d{2})[\.\-_/]", source_url)
    ...
    def _insert_inline_citations(
        ...
        # Line-based splitting: split by newlines only.
        # Splitting on "." causes citations to be injected mid-word (e.g. "V14[tag].2.2").
        # Lines already represent natural prose units in structured reports.
        raw_lines = text_for_processing.splitlines(keepends=False)
        ...
        for raw_line in raw_lines:
            ...
            sentence_segments.append((raw_line, "\n", sentence_text, is_matchable))
            if is_matchable:
        ...
        for cit_idx, cit in enumerate(citations_with_snippets):
            ...
            if coverage_ratio > 0.85 and has_snippet:
                min_threshold = 0.15  # High coverage: require meaningful connection
            else:
                min_threshold = 0.10  # ~25% keyword overlap sufficient to match
    ...
    def _build_reference_section(
        ...
        *tool_citations* (web_search / web_fetch / etc.). Model-invented plausible URLs are dropped.
        ...
        for title, url in inline_citation_pairs:
            ...
            CitationMiddleware._enrich_citation_dates_multi_strategy(
    ...
    def _process_state(self, state: CitationMiddlewareState) -> dict | None:
        ...
        if not citations_with_snippets:
            ...
            logger.debug("CitationMiddleware: messages count=%d", len(messages))
            for i, msg in enumerate(messages):
                if isinstance(msg, ToolMessage):
                    logger.debug("CitationMiddleware: ToolMessage[%d] name=%s content_len=%d", i, getattr(msg, "name", "?"), len(str(msg.content)))
            return None
