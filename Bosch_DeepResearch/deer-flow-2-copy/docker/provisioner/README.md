...
2. **Pod Creation**: The provisioner creates a dedicated Pod in the `deer-flow-2` namespace with:
...
|----------|---------|-------------|
| `K8S_NAMESPACE` | `deer-flow-2` | Kubernetes namespace for sandbox resources |
...
3. **Kubernetes Access**:
   - The provisioner needs permissions to:
     - Create/read/delete Pods in the `deer-flow-2` namespace
     - Create/read/delete Services in the `deer-flow-2` namespace
     - Read Namespaces (to create `deer-flow-2` if missing)
...
docker compose -p deer-flow-2-dev -f docker/docker-compose-dev.yaml up -d provisioner
```
...
docker exec deer-flow-2-provisioner curl -X POST http://localhost:8002/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001"}'

# Check sandbox status
docker exec deer-flow-2-provisioner curl http://localhost:8002/api/sandboxes/test-001

# List all sandboxes
docker exec deer-flow-2-provisioner curl http://localhost:8002/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n deer-flow-2 -l sandbox-id=test-001

# Delete sandbox
docker exec deer-flow-2-provisioner curl -X DELETE http://localhost:8002/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker exec deer-flow-2-provisioner curl -s http://localhost:8002/api/sandboxes/test-001 | jq -r .sandbox_url)

# Test from gateway container
docker exec deer-flow-2-gateway curl -s $SANDBOX_URL/v1/sandbox
```
...
- Verify inside container:
  ```bash
  docker exec deer-flow-2-provisioner ls -ld /root/.kube/config
  ```
...
- Verify the paths exist on your host machine:
  ...
  ls -la /path/to/backend/.deer-flow-2/threads
  ```
...
- Check Pod events: `kubectl describe pod sandbox-XXX -n deer-flow-2`
...
**Solution**:
- Verify the Service exists: `kubectl get svc -n deer-flow-2`
...
3. **Network Isolation**: Sandbox Pods run in the `deer-flow-2` namespace but share the host's network namespace via NodePort. Consider NetworkPolicies for stricter isolation.
