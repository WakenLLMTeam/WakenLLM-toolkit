...
Given a single slide outline, decide whether a visualization would help the audience.

{viz_density_instruction}
- title and section slides NEVER get viz.
...
LAYOUT OPTIONS — also decide the page layout based on information density:
  ...
  "two_col"  — Dual-column text, no viz. Use sparingly — only when content is pure narrative
               prose with no structured data, no steps, and no comparable items. needs_viz must be false.
...
If no viz is needed (title/section slides, or density constraint prohibits it):
{
  "slide_number": <N>,
  "needs_viz": false,
...
def _normalize_viz_spec(viz: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Also normalises type aliases (e.g. 'matrix' → 'matrix_2x2').
    """
    import copy
    viz = copy.deepcopy(viz)
    vt = (viz.get("type") or "").lower()

    # Resolve type aliases first so downstream checks see canonical names
    if vt in _TYPE_ALIASES:
        vt = _TYPE_ALIASES[vt]
        viz["type"] = vt
...
def _build_viz_density_strings(viz_density: float) -> tuple[str, str]:
    """
    Return (system_instruction, reminder) strings tailored to viz_density [0.0–1.0].

    viz_density=0.0 → no viz (caller should skip LLM entirely, but strings are safe)
    viz_density=1.0 → every content slide MUST have a viz
    intermediate   → target percentage injected into prompt
    """
    pct = int(round(viz_density * 100))
    if viz_density >= 1.0:
        instruction = (
            "VIZ DENSITY = 1.0 (MAXIMUM): EVERY content/summary slide MUST have a visualization.\n"
            "needs_viz MUST be true — returning needs_viz: false is NOT allowed for content slides."
        )
        reminder = (
            "MANDATORY: needs_viz MUST be true. Every content slide requires a chart."
        )
    elif viz_density >= 0.7:
        instruction = (
            f"VIZ FREQUENCY TARGET: ~{pct}% of content slides should have a visualization.\n"
            "- Strongly prefer adding a viz to every content slide.\n"
            "- Only skip viz when the slide is pure narrative prose with no structure.\n"
            "- When in doubt → ADD IT. Default to yes."
        )
        reminder = (
            f"REMINDER: needs_viz should be TRUE for ~{pct}% of content slides. "
            "An imperfect chart beats a text wall."
        )
    elif viz_density >= 0.4:
        instruction = (
            f"VIZ FREQUENCY TARGET: ~{pct}% of content slides should have a visualization.\n"
            "- Add a viz when it meaningfully structures the information.\n"
            "- Skip viz for conceptual or narrative slides where no chart type fits well."
        )
        reminder = (
            f"REMINDER: aim for approximately {pct}% of content slides to have a chart."
        )
    elif viz_density > 0.0:
        instruction = (
            f"VIZ FREQUENCY TARGET: only ~{pct}% of content slides should have a visualization.\n"
            "- Add a viz ONLY for the most data-rich, structured slides.\n"
            "- Prefer text-only for most slides — charts are the exception, not the default."
        )
        reminder = (
            f"REMINDER: be selective — only ~{pct}% of slides get a chart. "
            "Skip viz unless the content is clearly data-driven."
        )
    else:
        # viz_density == 0.0 — caller should skip LLM, but just in case
        instruction = (
            "VIZ DENSITY = 0.0: Do NOT add any visualization. "
            "needs_viz MUST be false for all slides."
        )
        reminder = "needs_viz MUST be false — no charts allowed at this density setting."
...
def _decide_slide_viz(
    ...
    density_instruction, density_reminder = _build_viz_density_strings(viz_density)

    # Use replace() — not .format() — because the system prompt contains JSON
    # examples with curly braces that would confuse str.format().
    system = _SLIDE_VIZ_DECISION_SYSTEM.replace("{viz_density_instruction}", density_instruction)
    user = _SLIDE_VIZ_DECISION_USER_TEMPLATE.format(
        ...
        viz_density_reminder=density_reminder,
    )
        ...
        if error:
            ...
            try:
                ...
                if isinstance(decision, dict):
                    decision.setdefault("slide_number", sn)
                    # Normalize + validate retry result
                    fig_retry = decision.get("figure") or {}
                    viz_retry = fig_retry.get("viz")
                    if viz_retry and isinstance(viz_retry, dict):
                        viz_retry = _normalize_viz_spec(viz_retry)
                        fig_retry["viz"] = viz_retry
                        retry_error = _validate_viz_spec(viz_retry)
                        if retry_error:
                            print(f"[slides_agent]   retry also invalid ({retry_error}), abandoning viz", file=sys.stderr)
                            decision["needs_viz"] = False
                            decision.pop("figure", None)
                else:
                    # Retry returned non-dict — keep original decision with no viz
...
def build_slides_pptx(
    ...
    Args:
        ...
        viz_density: Chart density [0.0–1.0].
                     0.0 = no charts (pure text PPT),
                     1.0 = every content slide has a chart,
                     0.7 = default (70% target).

    Returns:
        Absolute path to the generated .pptx file
    """
    viz_density = max(0.0, min(1.0, float(viz_density)))
    if backend is None:
    ...
    prev_layout: str = "default"
    _density_label = f"{int(viz_density*100)}%"
    print(f"[slides_agent] viz_density={viz_density} ({_density_label} chart target)", file=sys.stderr)

    for i, slide in enumerate(slide_list):
        sn = slide.get("slide_number", i + 1)
        stype = (slide.get("type") or "content").lower()
        is_content = stype in ("content", "summary")
        print(f"[slides_agent] Slide {sn}: deciding viz…", file=sys.stderr)

        # viz_density=0 → skip all viz decisions entirely (pure text mode)
        if viz_density == 0.0 or not is_content:
            if not is_content:
                pass  # title/section never get viz — fall through normally
            else:
                slide.setdefault("figure", {})
                slide["figure"]["image_path"] = ""
...
def main() -> int:
    ...
    parser.add_argument("--viz-density", dest="viz_density", type=float, default=0.5,
                        metavar="DENSITY",
                        help=(
                            "Chart density [0.0–1.0] (default: 0.7). "
                            "0.0 = pure text PPT (no charts), "
                            "1.0 = every content slide gets a chart, "
                            "0.5 = ~50%% of content slides."
                        ))
