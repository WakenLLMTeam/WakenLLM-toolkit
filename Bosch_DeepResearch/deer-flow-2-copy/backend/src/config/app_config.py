...
class AppConfig(BaseModel):
    ...
    def from_file(cls, config_path: str | None = None) -> Self:
        ...
        if "rag" in config_data:
            load_rag_config(config_data["rag"])
    ...
    def resolve_env_variables(cls, config: Any) -> Any:
        ...
        if isinstance(config, str):
            # Simple "$VAR" syntax for environment-variable substitution.
            # NOTE:
            # - For REQUIRED secrets, the corresponding model/tool code should
            #   validate and raise a clear error if the value is missing.
            # - Here we avoid crashing the whole application just because an
            #   optional provider (e.g. a model you are not using) is not
            #   configured in the current environment.
            if config.startswith("$"):
                env_name = config[1:]
                env_value = os.getenv(env_name)
                if env_value is None:
                    # Keep the original placeholder so downstream code can
                    # decide how to handle the missing variable (e.g. by
                    # treating this provider as disabled).
                    #
                    # This prevents hard failures like:
                    #   ValueError: Environment variable VOLCENGINE_API_KEY not found
                    # when the user simply hasn't configured that provider.
                    return config
                return env_value
            return config
