...
├── orchestrator/               # Three-phase consulting report pipeline (standalone)
├── middlewares/                # Repo-root middleware layer (shadows harness built-ins)
├── comfy_ppt_agent/            # ComfyUI image generation + PPTX assembly
...
│   │           │   ├── middlewares/   # Harness built-in middleware components
...
│   │   │   └── routers/       # models, mcp, memory, skills, uploads, threads, artifacts,
│   │   │                      # agents, suggestions, channels, timeline_ppt
│   │   └── channels/          # IM platform integrations
│   ├── tests/                 # Test suite
│   └── docs/                  # Documentation
├── frontend/                   # Next.js frontend application
├── scripts/
│   ├── ppt/                   # PPT dev & experiment scripts (not production)
│   ├── dev/                   # Developer utilities (e.g. export_claude_code_oauth.py)
│   └── *.sh / *.py            # System/infra scripts called by Makefile
...
1. **ThreadDataMiddleware** - Creates per-thread directories (`backend/.deer-flow/threads/{thread_id}/user-data/{workspace,uploads,outputs}`); Web UI thread deletion now follows LangGraph thread removal with Gateway cleanup of the local `.deer-flow-2/threads/{thread_id}` directory
...
| **Timeline PPT** (`/api/ppt/timeline`) | `POST /l2-l3` - generate L2→L3 ADAS evolution PPTX (built-in spec); `POST /generate` - custom `deck_spec` JSON body; delegates to `skills/public/timeline-evolution-ppt/scripts/orchestrate_timeline_pptx.py` as subprocess |
...
- Physical: `backend/.deer-flow-2/threads/{thread_id}/user-data/...`, `deer-flow-2/skills/`
...
- **Location**: `deer-flow-2/skills/{public,custom}/`
...
**Data Structure** (stored in `backend/.deer-flow-2/memory.json`):
