...
from src.rag.retriever import Chunk, Document, MetadataCondition, MetadataField, Resource, Retriever
...
class RAGFlowProvider(Retriever):
    ...
    @staticmethod
    def _merge_retrieval_result(data: dict, docs: dict[str, Document]) -> None:
        ...
        for chunk in data.get("chunks", []):
            ...
            if getattr(doc, "dataset_id", None) is None and chunk.get("dataset_id"):
                setattr(doc, "dataset_id", chunk.get("dataset_id"))
            doc.chunks.append(
                Chunk(
